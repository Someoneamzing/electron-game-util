...
class GUI extends EventEmitter {
  ...
  getInitialData(object) {
    return Object.keys(this.elements).map(name=>{
      ...
      return [name, Object.keys(element.getters).map(e=>{return {type: "getters", prop: e}}).concat(Object.keys(element.setters).map(e=>{return {type: "setters", prop: e}})).filter(propData=>element[propData.type][propData.prop]?true:false).reduce((out, propData)=>{out[propData.prop] = element[propData.type][propData.prop].value;return out;}, {})];
    })
  }
    ...
    if (this.server.side == ConnectionManager.SERVER) {
      ...
      if (!(object.netID in this.watchedObjects)) {
        ...
        for (let name in this.elements) {
          ...
          for (let propertyName in element.getters) {
            let property = element.getters[propertyName];
          ...
          for (var prop in element.setters) {
            //// TODO: add setters functionality
          }
        }
      } else {
      ...
      for (let data of socket.initialData) {
        for (let propData in data[1]){
          switch (propData.type) {
            case "getters":
              console.log(prop);
              this.elements[data[0]].setValue(propData.prop, data[1][propData.prop]);
              break;
            case "setters":
              // this.elements[data[0]].setters[propData.prop]
              break;
          }
  ...
  close(socket) {
    ...
    if (this.server.side == ConnectionManager.SERVER) {
      ...
      if (this.watchedObjects[object.netID].sockets.length == 0) {
        for (let name in this.elements) {
          let element = this.elements[name];
          for (let propertyName in element.properties) if (element.properties[propertyName] && element.properties[propertyName].type != 'constant') object.unwatch(element.properties[propertyName].property);
        }
